  ...
  <a-card :body-style="{padding: '24px 32px'}" :bordered="false">
    ...
    >
      ...
      <a-form-item
        ...
        :wrapperCol="{span: 10}"
      >
        <a-input disabled="true" :placeholder="$t('emailInput')" v-decorator="[
          'email',
...
export default {
  ...
  async mounted() {
    await getRoleList(data => (this.roles = data));
    await UserService.getUser(this.$route.params.id).then((request) => {
      let res = request.data;

      let roleId = request.data.data.roles[0].id;
      let userData = {roleId: roleId, username: request.data.data.username, email: request.data.data.email}
      if (res.code > 0) {
        this.form.setFieldsValue(userData)
      } else {
